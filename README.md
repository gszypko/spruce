...
- `equation_set`: One of `{ideal_mhd}`. Selects the `EquationSet` to be used for the simulation, which defines the system of equations being solved, stores the relevant quantities, and performs all necessary time derivative calculations. This must be defined **before** `output_flags` and before any active `Module`s.
- `output_flags`: A comma-separated list composed of any of the variable names defined in the specified `EquationSet` (for `IdealMHD`, that is `{d_x,d_y,pos_x,pos_y,rho,temp,mom_x,mom_y,be_x,be_y,grav_x,grav_y,press,thermal_energy,kinetic_energy,div_bi,bi_x,bi_y,dt,v_x,v_y,n,div_be,b_hat_x,b_hat_y,b_magnitude,v_a}`). Specifies the variables that are written to the `mhd.out` file whenever required.
