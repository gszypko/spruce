...
inp = {'folder';
    'C:\Users\Grant\OneDrive\Research\mhd\data-expsims\an-mhd-09.26.22\HIH-2';
      };
s = spreadsheet2struct(inp,inp(1,:));

% loading options
flags.sim.time_window = [0 100]; % [start end] percentage of time window to load
flags.sim.time_interval = 10; % interval for time indices to keep
...
flags.exp.t_max = 1; % units of tau
flags.exp.num_time_pts = 500; % sets time interval for recording grids during simulation
flags.exp.apply_sg_filt = false; % whether or not to apply an SG filter to the density distribution
...
flags.exp.sim_domain = [0.8 0.8]; % [x y] domain limits in cm, symmetric about origin
flags.exp.num_grids = 301; % number of points on x axis for simulation domain
flags.exp.n_dist = 'gauss'; % gauss or exp
flags.exp.n_min = 1e-4*966028381.2939; % minimum in simulation density distribution cm^-^3
flags.exp.is_uniform = false;
flags.exp.grid_growth = 1.07;
flags.exp.grid_spread = 0.99999;
flags.exp.apply_sg_bgd = true; % whether or not to apply an SG filter to the density distribution
flags.exp.sg_bgd_length = 0.05; % length scale (cm) for SG kernel when filtering density distribution
flags.exp.sg_radius = 0.5; % radius outside of which the background sg filter is applied
flags.Te = [37.4]; % electron temperature (K) to use for this data set (if empty, use Te in exp data settings)
