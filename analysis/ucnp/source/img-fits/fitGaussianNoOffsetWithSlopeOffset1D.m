function [results] = fitGaussianNoOffsetWithSlopeOffset1D(x,y)
...
L = @(x,slope,cen) slope.*(x-cen);
fit_model = @(p,x) G(x,p(1),p(2),p(3)) + L(x,p(4),p(5));
...
p0(4) = 0; lb(4) = -inf; ub(4) = inf;
p0(5) = 0; lb(5) = -sig/2; ub(5) = sig/2;
