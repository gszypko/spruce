...
MhdInp gen_inp_grids_ucnp(const PlasmaSettings& pms)
{
    ...
    if (strcmp(pms.getopt("n_dist"),"gaussian")){
        grids.set_var(PlasmaDomain::rho,gaussian2D(x,y,rho_max,pms.getvar("sig_x"),pms.getvar("sig_y"),0,0));
    }
    else if (strcmp(pms.getopt("n_dist"),"exponential")){
        grids.set_var(PlasmaDomain::rho,exponential2D(x,y,rho_max,pms.getvar("sig_x"),pms.getvar("sig_y"),0,0));
    }
